...
### `src/ui/main.tsx`: start the visible application
...
### `src/ui/App.tsx`: types and constants
...
### `src/api/*.ts`: talking to Supabase

All three repository files follow the same readable pattern:

1. Check `if (!supabase)`. When environment settings are missing, loading
   returns a safe empty value and saving throws a clear configuration error.
2. Start with `supabase.from('table_name')` to choose a table.
3. Use `.select(...)` to read, `.upsert(...)` to insert-or-update, or
   `.delete()` to remove.
4. Use `.eq(...)` to limit an operation to one ID.
5. Check the returned `error` and throw it instead of silently pretending the
   database operation worked.
6. Convert database column names such as `start_minutes` into frontend names
   such as `startMinutes`.
7. Subscription functions open a real-time channel and call `onChange` after
   an insert, update, or delete.
8. The function returned by a subscription removes that channel. React calls
   this cleanup when the component closes, preventing duplicate listeners.

`scheduleRepository.ts` handles the main imported schedule.
`adminEventRepository.ts` handles individual special events.
`studentAssistantRepository.ts` handles assistant uploads and solver results.

### `src/api/studentAssistantSolver.ts`: send work to Python
...
### `src/electron/main.ts`: create the desktop program
...
### `src/electron/preload.ts` and `src/electron/util.ts`
...
### `python/app.py`: receive the browser request
...
### `python/solver/student_assistant_solver.py`: build the schedule
...
- `vite.config.ts` configures the web build and Electron development process.
